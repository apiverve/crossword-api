...
class CrosswordValidationException implements Exception {
  final List<String> errors;

  CrosswordValidationException(this.errors);

  @override
  String toString() => 'CrosswordValidationException: ${errors.join("; ")}';
}
...
/// Crossword Generator API client.
///
/// For more information, visit: https://apiverve.com/marketplace/crossword?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [size] - Grid size: small, medium, large
/// * [theme] - Theme: random, animals, food, sports, science, geography
...
class CrosswordClient {
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'size': ValidationRule(type: 'string', required: false),
    'theme': ValidationRule(type: 'string', required: false),
    'difficulty': ValidationRule(type: 'string', required: false),
  };

  CrosswordClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/crossword',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [CrosswordValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw CrosswordValidationException(errors);
    }
  }

  /// Execute a request to the Crossword Generator API.
  ///
  /// Parameters are validated before sending the request.
  Future<CrosswordResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw CrosswordException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return CrosswordResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw CrosswordException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw CrosswordException('Resource not found');
      } else {
        throw CrosswordException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is CrosswordException) rethrow;
      throw CrosswordException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Crossword Generator API client.
class CrosswordException implements Exception {
  final String message;

  CrosswordException(this.message);

  @override
  String toString() => 'CrosswordException: $message';
}
